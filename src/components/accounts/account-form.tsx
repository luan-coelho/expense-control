...
export function AccountForm({ account, onSuccess, onCancel }: AccountFormProps) {
  ...
  const isNameValid = nameLength >= ACCOUNT_VALIDATION_RULES.NAME_MIN_LENGTH && 
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormDescription className="text-xs">
                Escolha o tipo que melhor representa esta conta
              </FormDescription>
              <FormMessage />
            </FormItem>
          )}
        ...
        {nameLength > 0 && (
          <div className="rounded-lg bg-muted/50 p-3 space-y-2">
            ...
            <div className="space-y-1 text-xs">
              ...
              <div className={`flex items-center gap-2 ${
                ACCOUNT_VALIDATION_RULES.ALLOWED_NAME_PATTERN.test(watchedName || '') ? 'text-green-600' : 'text-muted-foreground'
              }`}>
          ...
          <div className="rounded-lg bg-muted/50 p-3">
            ...
            <div className="flex items-center gap-2 text-sm">
              <span className="text-lg">
                {accountTypes.find(t => t.value === watchedType)?.icon}
              </span>
              <span className="font-medium">
                {accountTypes.find(t => t.value === watchedType)?.label}
              </span>
            </div>
          </div>
        )}
